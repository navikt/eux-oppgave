...
class OppgaverApiValidationTest : AbstractOppgaverApiImplTest() {
    ...
    @Test
    fun `POST oppgaver - forespørsel, ugyldig tildeltEnhetsnr - 400`() {
    ...
    @Test
    fun `POST oppgaver - forespørsel, ugyldig opprettetAvEnhetsnr - 400`() {
        val createResponse = restTestClient
            .post()
            .uri(oppgaverUrl)
            .header("Authorization", "Bearer ${mockOAuth2Server.token}")
            .body(oppgaverOpprettelse.copy(opprettetAvEnhetsnr = "12345"))
            .exchange()
            .expectStatus().isEqualTo(400)
            .expectBody(MethodArgumentNotValidExceptionAdvice.ApiError::class.java)
            .returnResult()
    ...
    @Test
    fun `POST oppgaver tildelEnhetsnr - forespørsel, ugyldig tildeltEnhetsnr - 400`() {
        val createResponse = restTestClient
            .post()
            .uri(oppgaverTildelEnhetsnrUrl)
            .header("Authorization", "Bearer ${mockOAuth2Server.token}")
            .body(oppgaverTildelEnhetsnrDataset.copy(tildeltEnhetsnr = "12345"))
            .exchange()
            .expectStatus().isEqualTo(400)
            .expectBody(MethodArgumentNotValidExceptionAdvice.ApiError::class.java)
            .returnResult()
